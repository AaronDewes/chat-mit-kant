    ...
    <div class="flex gap-2 flex-col w-full flex-grow overflow-scroll">
      <template v-for="message in history" :key="message.id">
        <div v-if="message.sender === 'USER'" class="rounded-md rounded-br-none max-w-3/4 ml-auto flex bg-green-500 p-4">
          <span>{{
              message.text.replace("Bitte beantworte die folgende Frage auf Deutsch, benutze kein Englisch. Gehe in deiner Antwort nicht auf den Text vor den --- ein, sondern beantworte bitte nur die Frage. ---", "")
            }}</span>
        </div>
        <div v-if="message.sender === 'CLONE'" class="rounded-md rounded-bl-none max-w-1/2 mr-auto bg-teal-500 p-4">
        <span>{{
            message.text == "Greetings, I'm Immanuel Kant. What philosophical questions are you pondering today?" ? "Hallo, ich bin Immanuel Kant. Welche philosophischen Fragen beschäftigen Sie heute?" : message.text
          }}</span>
        </div>
      </template>
    </div>
